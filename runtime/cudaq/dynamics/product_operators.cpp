...
template<typename HandlerTy>
int product_operator<HandlerTy>::term_count() const { 
...
template
cudaq::matrix_2 product_operator<elementary_operator>::m_evaluate(
    MatrixArithmetics arithmetics, std::map<int, int> dimensions,
    std::map<std::string, std::complex<double>> parameters, bool pad_terms);
...
template
int product_operator<elementary_operator>::term_count() const;
